    ...
    <div class="filter-container">
      ...
      <el-dialog :title="InsertDiagInfo.title"
        ...
        <InsertDiagUser :visible.sync="InsertDiagInfo.visible"
                        @getTableData="getTableData('admin')">
          <!-- :visible.sync双向绑定数据
      ...
      <el-table-column label="操作"
        ...
        <template slot-scope="scope">
          <el-button size="mini"
                     ...
                     type="info"
                     plain
                     @click="handleCancel(scope.row.userId, scope.row.userNum)">注销</el-button>
          <el-button size="mini"
                     v-else
                     type="warning"
                     plain
          ...
          <el-button size="mini"
                     type="danger"
                     plain
                     @click="handleDelete(scope.row.userId, scope.row.userNum, scope.row.userRealName, scope.row.role.roleName, scope.row.role.token)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div class="block">
...
export default {
  ...
  mounted: function () {
    this.getTableData('admin') //需要触发的函数
  },
  methods: {
    ...
    // 处理插入操作
    handleCreate () {
      this.InsertDiagInfo.title = '新增管理员' // 设置题目
    ...
    handleUpdate (userId) {
      ...
      this.UpdateDiagInfo.title = '编辑管理员' // 设置题目
    ...
    // 处理注销操作
    handleCancel (userId, userNum) {
      if (userNum === process.env.VUE_APP_ADMIN) {
        this.$message({
          message: '此账户不能注销！！!',
          type: 'error'
        })
    ...
    // 处理删除操作, 不可逆操作, 注意！！！！！！
    handleDelete (userId, userNum, userRealName, roleName, token) {
