  ...
  <el-form ref="form"
    ...
    <el-form-item label="教师">
      <el-select v-model="form.teacherId">
        <el-option v-for="item in form.teachers"
                   :key="item.userId"
    ...
    <el-form-item label="班级">
      <el-select v-model="form.classId">
        <el-option v-for="item in form.classes"
                   :key="item.classId"
...
export default {
  ...
  data () {
    ...
    var validateUserNum = (rules, value, callback) => {
      ...
      if (!pathRegExp.test(value)) {
        return callback(new Error('请输入12位数字的学号'));
      } else {
        callback();
      }
    };
    ...
    return {
      form: {
        ...
        teachers: [], // select下拉框教师信息
        classes: [], // select下拉框班级信息
        teacherId: '', // 选中的教师
        classId: '', // 选中的班级
        // 状态（查询选择）
        defFlags: [
  ...
  created: function () {
    ...
    this.getTeachers() // 获取所有的教师
    this.getClasses() // 获取所有的班级
  ...
  methods: {
    ...
    getClasses () {
      request({
        ...
        let data = res.data
        this.form.classes = data
      })
    },
    // 获取所有的教师
    getTeachers () {
      request({
        url: '/api/tb-user/select_all_teacher',
        method: 'get',
      }).then(res => {
        let data = res.data
